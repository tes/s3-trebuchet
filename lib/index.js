...
module.exports = ({
  ...
  const fileValidationHandler = (fileKeyParamName) => {
    if (!fileKeyParamName) {
      throw new Error('You need to specify which route parameter will contain the fileKey');
    }
    return async (req, res, next) => {
      try {
        const fileKey = req.params[fileKeyParamName];
        const result = await validateFileUpload(fileKey);
        return res.status(200).send(result);
      } catch (error) {
        error.statusCode ? next(Boom.boomify(error)) : next(Boom.badRequest(error.message));
  ...
  const goToTemporaryUrlForFileHandler = (fileKeyParamName) => {
    if (!fileKeyParamName) {
      throw new Error('You need to specify which route parameter will contain the fileKey');
    }
    return async (req, res, next) => {
      try {
        const fileKey = req.params[fileKeyParamName];
        const url = await getTemporaryUrlForFile({ fileKey });
        res.redirect(url);
      } catch (error) {
        console.log(error);
